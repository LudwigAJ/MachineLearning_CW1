...
Inside the CO395-CW.py there is a function named main. Inside this function we call runTest() which takes three parameters. The first one specifies the path to the training dataset, the 2nd parameter specifies the number of folds, and the last parameter takes in a boolean specifying if we should print the tree while its created or not. We did not include any command-line arguments.
